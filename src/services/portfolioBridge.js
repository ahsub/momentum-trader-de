/**
 * Portfolio Bridge – Verbindet CapTrader-Daten mit dem Options-Screener
 */
...
export function calculatePortfolioMetrics(parsedData) {
  ...
  const maxPosition = positions.length > 0 
    ? Math.max(...positions.map(p => p.percentOfNAV)) 
    : 0;
  ...
  return {
    ...
    maxConcentration: maxPosition,
    bySubCategory,
    positions,
    portfolioDelta: totalEquity * 0.01,
    portfolioTheta: 0,
  };
}
  ...
  Object.entries(grouped).forEach(([key, trades]) => {
    ...
    if (netQty !== 0) {
      ...
      openOptions.push({
        ...
        netQuantity: netQty,
        isShort: netQty < 0,
        daysToExpiry: first.expiry 
          ? Math.ceil((new Date(first.expiry) - new Date()) / (1000 * 60 * 60 * 24))
          : null,
      });
    }
  });
...
export { extractUnderlyings, calculatePositionsWithCostBasis } from './capTraderParser';
